...
def get_visited_positions(moves):
    ...
    for move in moves:
        ...
        amount = int(move[1:])
        if direction == 'L': 
            positions.update([(x - step, y) for step in range(1, amount+ 1 )])
            x -= amount
        if direction == 'R': 
            positions.update([(x + step, y) for step in range(1, amount+ 1 )])
            x += amount
        if direction == 'U': 
            positions.update([(x, y + step) for step in range(1, amount+ 1 )])
            y += amount
        if direction == 'D': 
            positions.update([(x, y - step) for step in range(1, amount+ 1 )])
            y -= amount
...
crossings = find_crossing([move for move in INPUT_1.split(',')], [move for move in INPUT_2.split(',')])
distances = [abs(crossing[0]) + abs(crossing[1]) for crossing in crossings]
print(min(distances))