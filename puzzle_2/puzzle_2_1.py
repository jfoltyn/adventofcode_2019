...
def execute_program(program):
    ...
    while True:
        ...
        if instruction == 99:
            ...
            x_pos = memory[pc + 1]
            y_pos = memory[pc + 2]
            result_pos = memory[pc + 3]
            x = memory[x_pos]
            y = memory[y_pos]
            memory[result_pos] = x+y
            pc+=4
...
result_memory = execute_program([int(value) for value in PROGRAM.split(',')])
print(f'Value at memory position 0 = {result_memory[0]}')